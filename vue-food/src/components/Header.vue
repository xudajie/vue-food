<template>
  <header>
  	<nav class="navbar navbar-expand-lg navbar-light bg-light">
	    ...
	    <a href="#" class="navbar-brand">Pizza点餐服务</a>
	    <ul class="navbar-nav">
	    	<li><router-link :to="{name:'homeLink'}" class="nav-link">主页</router-link></li>
	    	<!-- v-bind:to="{name:'路由名'}"利用路由的名字，也可以实现路由跳转 -->
	    ...
	    <ul class="navbar-nav ml-auto">
	    	...
	    	<li>
	    		<router-link to="register" v-show="!isLogin" class="nav-link">注册</router-link>
	    	</li>
	    	<li class="nav-link">
	    		{{currentUser}}
	    	</li>
	    	<li>
	    		<router-link to="login" v-show="isLogin" class="nav-link">[退出]</router-link>
	    	</li>
	    </ul>
  	</nav>
  </header>
